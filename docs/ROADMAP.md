...
- Per-zone data export bundle (Postgres dump + uploaded files + reports archive). *(PR 1 + PR 2 backend landed: `zone_exports` schema, zone-scoped table registry with FK-order coverage test, streamed `tar.gz` bundle generator under a REPEATABLE READ snapshot, owner-only `POST/GET/download` routes with a 24h per-zone cooldown, pg-boss queue + email + boot-sweep mirroring the report-jobs pattern, and a daily expiry sweep at `0 5 * * *`. PR 3 will add the `/zone/settings` UI alongside retention and the `scripts/restore-export.ts` round-trip helper that proves the exit criterion.)*
