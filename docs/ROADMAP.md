...
- GDPR data subject request workflow (export and erase). *(PR 1 of 3 landed: `erasure_requests` table (migration `0022_funny_kat_farrell.sql`) carries the `pending → applied / cancelled / failed` state machine for both member-scope (single PII scrub) and zone-scope (export-then-decommission) erases, with partial unique indexes enforcing at-most-one-pending per (zone, member) and per zone, and a CHECK that scope `member` rows always carry a `member_id` while `zone` rows never do. The export-bundle registry now includes `erasure_requests` at restore-order 930 so an operator's bundle carries the erase history alongside the audit log. Service layer + routes + UI follow in PRs 2 and 3.)*
