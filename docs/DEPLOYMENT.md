...
- **Restoring**: `pnpm --filter @stewardledger/api restore-export <bundle.tar.gz> [--target-zone-id <uuid>] [--target-slug <slug>] [--map-users <map.json>] [--storage-root <dir>] [--dry-run]` restores a bundle into a fresh zone identity in a target database (Drizzle-migrated to the bundle's `formatVersion` first). The helper rewrites `zone_id` + storage-key prefixes, nulls every `*_by_user_id` column by default (override with `--map-users`), and re-uploads import + report blobs at the new zone keys. The round-trip is exercised by `packages/api/src/services/exports/restore.test.ts`.
  - **User-FK remapping**: Better Auth identities (the `user` table) are intentionally **not** in the bundle — the restore-helper either nulls `*_by_user_id` columns (default) or remaps them via `--map-users <map.json>` (shape: `{"sourceUserId": "targetUserId"}`). Rows whose `_user_id` is `NOT NULL` (e.g. `report_jobs.user_id`, `roles.created_by_user_id`, `user_role_bindings.user_id`) are **skipped** when no map entry is supplied — inserting them would violate the FK against the target's empty `user` table. To restore those rows, pre-seed the referenced users in the target deployment (e.g. by re-running the invite flow against the new tenant) and pass a `--map-users` JSON file that maps every source user id to its target counterpart. The skipped count surfaces in the per-table summary the helper prints on completion.
