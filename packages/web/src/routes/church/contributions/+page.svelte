<script lang="ts">
  // Chapter-scoped contributions journal. Forks /zone/contributions but
  // strips the chapter selector (we always filter to the active chapter)
  // and the all-chapters totals card. The new-batch flow still hands off
  // to /zone/contributions/batches/new — that screen accepts ?chapterId=
  // so we pre-fill it.
  ...
  async function newBatch() {
    const here = chapter();
    if (!here) return;
    await goto(`/zone/contributions/batches/new?chapterId=${encodeURIComponent(here.id)}`);
  }

  // Posted totals grouped by currency. The chapter scope already constrains
  // the rows, so this is the chapter's posted figure for the current filter.
...
<svelte:head><title>Contributions · {chapter()?.name ?? "Chapter"} · StewardLedger</title></svelte:head>

<div>
  <div class="sl-reveal sl-reveal-1 flex flex-wrap items-end justify-between gap-6">
    <div>
      <span class="sl-eyebrow">§ Chapter ledger · Treasurer</span>
  ...
  <div class="sl-reveal sl-reveal-2 mt-10 grid grid-cols-2 gap-0 border-y border-[var(--rule)] bg-[var(--card)] md:grid-cols-3">
    ...
    <div class="border-t border-[var(--rule)] px-6 py-7 md:border-r md:border-t-0 md:border-[var(--rule)]">
      ...
      <div class="mt-3 sl-display sl-num text-[28px] leading-tight text-[var(--ink)]">
        ...
        {:else}
          {#each batchCashTotal as total, i (i)}
    ...
    <div class="border-t border-[var(--rule)] px-6 py-7 md:border-t-0">
      ...
      <div class="mt-3 sl-display sl-num text-[28px] leading-tight text-[var(--brass-deep)]">
        {#if tab === "contributions" && postedByCurrency.length > 0}
          {#each postedByCurrency as total, i (i)}
            <span class:block={i > 0}>{formatMoney(total)}</span>
          {/each}
        {:else}
          <span class="text-[var(--ink-faint)]">—</span>
      ...
      <table class="sl-table">
        ...
        <tbody>
          {#each batches as b (b.id)}
            <tr>
              <td>
                <a href={`/zone/contributions/batches/${b.id}`} class="sl-mono text-[12px] text-[var(--ink)] hover:text-[var(--brass-deep)]">
              ...
              <td class="text-right sl-mono sl-num text-[var(--ink)]">{b.cashTotal ? fmt(b.cashTotal, b.currencyCode) : "—"}</td>
              <td class="text-right sl-mono sl-num text-[var(--ink)]">{b.chequeTotal ? fmt(b.chequeTotal, b.currencyCode) : "—"}</td>
      ...
      <table class="sl-table">
        ...
        <tbody>
          {#each contributions as c (c.id)}
            <tr>
              <td class="sl-mono text-[12px] text-[var(--ink)]">
                <a href={`/zone/contributions/${c.id}`} class="hover:text-[var(--brass-deep)]">{c.contributionDate}</a>
              </td>
          ...
          {#if !loading && contributions.length === 0}
            <tr><td colspan="5" class="py-12 text-center text-[13px] text-[var(--ink-mute)]">{chapter() ? "No contributions yet." : "Select a chapter to begin."}</td></tr>
          {/if}
        </tbody>
      </table>
    </div>
