<script lang="ts">
  // Chapter-scoped reports library. Same registry as /zone/reports — every
  // report is server-driven and discovers itself via /api/tenant/reports.
  // The links land on /zone/reports/[id] with chapterId pre-filled, so the
  // report parameters page can default to the active chapter.
  ...
  function reportHref(id: string): string {
    const here = chapter();
    return here ? `/zone/reports/${id}?chapterId=${encodeURIComponent(here.id)}` : `/zone/reports/${id}`;
  }
</script>

<svelte:head><title>Reports · {chapter()?.name ?? "Chapter"} · StewardLedger</title></svelte:head>

<div>
  <div class="sl-reveal sl-reveal-1">
    <span class="sl-eyebrow">§ Chapter output · Reports</span>
    <h1 class="mt-3 sl-display text-[52px] leading-[1] text-[var(--ink)]">
      {chapter()?.name ?? "Chapter"} <span class="sl-serif-italic font-light text-[var(--brass-deep)]">reports</span>
    </h1>
    <p class="mt-3 max-w-xl text-[15px] text-[var(--ink-mute)]">
      Statements, ledgers, and member reports — pre-scoped to your chapter.
      Open a report and tweak the filters before exporting.
    </p>
  </div>
  ...
  <div class="sl-reveal sl-reveal-2 mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
    {#each items as report, i (report.id)}
      ...
      >
        <span class="sl-mono absolute right-5 top-5 text-[10.5px] text-[var(--ink-faint)]" style="letter-spacing:0.1em">
