<script lang="ts">
  ...
  async function reverseRow() {
    ...
    try {
      ...
      await goto(`/contributions/${res.contribution.id}`);
    } catch (err) {
  ...
  async function deleteDraft() {
    ...
    try {
      ...
      if (contribution.batchId) {
        await goto(`/contributions/batches/${contribution.batchId}`);
      } else {
  ...
  {:else}
    ...
    {#if contribution.batchId}
      <p class="mt-3 text-sm">
        Part of batch
        <a
          href={`/contributions/batches/${contribution.batchId}`}
    ...
    {#if contribution.reversalOfContributionId}
      <p class="mt-1 text-sm text-rose-700">
        Reversal of
        <a
          href={`/contributions/${contribution.reversalOfContributionId}`}
    ...
    <div class="mt-8 flex flex-wrap gap-2">
      ...
      {#if contribution.memberId}
        <a
          href={`/members/${contribution.memberId}/statement`}
