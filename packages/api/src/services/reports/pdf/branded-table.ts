...
/** Letter-size landscape constants. pdfkit defaults assume 72 dpi. */
...
export async function renderBrandedTablePdf(
  ...
  const pageWidth = landscape ? 792 : 612; // letter, 72 dpi
  ...
  // Data rows. We measure each row's height once so a wrapping cell
  // doesn't overflow the page band — `pdfkit`'s `heightOfString` is
  // the canonical way to do that.
  doc.font("Helvetica").fontSize(9).fillColor("#000000");
  const pageBottom = pageWidth === 792 ? 612 - PAGE_MARGINS.bottom : 792 - PAGE_MARGINS.bottom;
  for (const row of args.rows) {
    ...
    const rowHeight = Math.max(...heights, 11) + ROW_PADDING;
