...
export const memberStatementFiltersSchema = z.object({
  memberId: uuidSchema,
...
> = {
  ...
  accessCheck: (ctx, filters) => {
    ...
    // readers can only pull statements for members whose home chapter
    // is in their bindings. The row-fetch query also re-applies this
    // filter via `chapterId` so a misconfigured caller can't widen.
    if (isZoneRead(ctx)) return null;
    if (ctx.chapterIds.length === 0) return "forbidden";
    return null; // chapter scope is enforced via the row fetch's chapterId clause
  },
  async fetch(database, ctx, filters): Promise<ReportFetchResult<MemberStatementRow>> {
    if (filters.dateFrom > filters.dateTo) {
      // Tighten the loop: silently swap rather than 422 — small UX win.
      filters = { ...filters, dateFrom: filters.dateTo, dateTo: filters.dateFrom };
    }
    ...
    // Chapter-scoped users: deny if the member's home chapter isn't in
    // their bindings. Returning empty would hide PII silently; we
    // surface a forbidden via accessCheck-style 403 instead.
